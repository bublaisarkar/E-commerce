🛍️ E-commerce MERN Stack Project

Status Live

Live URL :https://rabbit-gray.vercel.app/

<img width="1365" height="639" alt="image" src="https://github.com/user-attachments/assets/3d8468bc-3e3f-4b9b-9f5a-15c8526a2808" />


📝 Description

A full-stack e-commerce application built using the MERN stack (MongoDB, Express, React, Node.js). It provides a complete shopping experience, including product browsing, user authentication, a shopping cart, and a simulated checkout process.

✨ Key Features

User Authentication: Secure Login and Registration for users.

Product Catalog: Browse, search, and filter a wide range of products.

Shopping Cart: Add, remove, and update product quantities in the cart.

Checkout Flow: Step-by-step process for shipping, payment (simulated), and final order placement.

Order History: Users can view details of their past orders.

Responsive Design: Optimized for desktop, tablet, and mobile viewing.

Admin Panel (Typical): Dashboard for managing products, users, and viewing order analytics.
